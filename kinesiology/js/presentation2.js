// =====================================================================
// presentation2.js
// 運動学実習 発表アンケート presentation2
// =====================================================================

// ★ GAS_URL を presentation2 用のエンドポイントに設定してください
const GAS_URL = "https://script.google.com/macros/s/AKfycby2KEhI3NgIwKsfG17ey4iLqb5EIpuIvdsLYjOOca6GJkbRv8H7vq9dKEq8cdM_BS0z/exec";
...
function initForm() {
  for (let i = 0; i < 3; i++) {
    createEvalBlock("walk-section", "walk", i);
    createEvalBlock("jump-section", "jump", i);
  }
...
function validateForm() {
  ...
  const walk = getSectionValues("walk");
  const jump = getSectionValues("jump");
  ...
  errors.push.apply(errors, validateSection(walk, ownGroup, "歩行"));
  errors.push.apply(errors, validateSection(jump, ownGroup, "立ち上がり・ジャンプ"));
...
function submitForm() {
  ...
  if (!GAS_URL || GAS_URL.includes("ここに") || GAS_URL.includes("YOUR_GAS")) {
    if (statusEl) {
      statusEl.textContent = "※ 送信先未設定（js/presentation2.js の GAS_URL を設定してください）";
  ...
  params.append("type", "presentation2");
  params.append("studentId", result.studentId);
  params.append("ownGroup", result.ownGroup);

  params.append("walkGroup1", result.walk[0].group);
  params.append("walkComment1", result.walk[0].comment);
  params.append("walkGroup2", result.walk[1].group);
  params.append("walkComment2", result.walk[1].comment);
  params.append("walkGroup3", result.walk[2].group);
  params.append("walkComment3", result.walk[2].comment);

  params.append("jumpGroup1", result.jump[0].group);
  params.append("jumpComment1", result.jump[0].comment);
  params.append("jumpGroup2", result.jump[1].group);
  params.append("jumpComment2", result.jump[1].comment);
  params.append("jumpGroup3", result.jump[2].group);
  params.append("jumpComment3", result.jump[2].comment);
